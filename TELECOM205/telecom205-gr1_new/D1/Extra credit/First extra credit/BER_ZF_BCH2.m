...
for ii = 1:length(SNR_bit);
  ...
  while ( ((nb_err_ZF(ii)< nb_err_min) && (jj < nb_iter_max )))   
    ...
    for ll = 1:loop_number_coding;
      bits_est = d_est_ZF(1,(((ll-1)*N_bits_code)+1):ll*N_bits_code);
    ...
    ending_uc = d_est_ZF(1,(loop_number_coding*N_bits_code+1):end);
    bits_final_ber = [bits_final_ber, ending_uc];
    
    % calcul du BER pour le treshold
    nb_err_ZF(ii)=nb_err_ZF(ii)+sum(abs(bits_final_ber-bits_final)); % compter le nombre d'erreurs

    jj = jj +1;
  end 
  BER_ZF(ii) = nb_err_ZF(ii)/(jj*N_bits);  % calcul du taux d'erreur binaire
