...
nb_of_loops = 0; %Pour compter le surplus de bits employés qd on fait 1 tour de plus
...
for ii = 1:length(SNR_bit);
  ...
  while ((jj < nb_iter_max)) 
    ...
    for qq = 0:ARQ_nb_of_reps;
      ...
      nb_err = sum(abs(bits_final_ber-bits_final));
  ...
  if(nb_err==0)
    nb_corr_treshold(ii) = nb_corr_treshold(ii)+(loop_number_coding*N_bits_info-nb_err);
  endif
  
  end 
  throughput_treshold(ii) = nb_corr_treshold(ii)/((jj+nb_of_loops)*N_bits);
end
