...
NbitsDAC = [1:1:20];           % Number of bits of the DAC
Vref    = 1;            % Voltage reference of the DAC
dacType = 'zoh';        % DAC type ; can be 'zoh' or 'impulse'
ACPR = zeros(1,length(NbitsDAC));
ACPR_high = zeros(1,length(NbitsDAC));
ACPR_low = zeros(1,length(NbitsDAC));
Pow_useful_band =zeros(1,length(NbitsDAC));
...
for ii=1:1:length(NbitsDAC)
    % Perform conversion
    [basebandAnalog_dac_I, DAC_I_pc] = PC_DAC(basebandDigital_I,NbitsDAC(ii),Vref,dacType,basebandSamplingRate,continuousTimeSamplingRate);
    [basebandAnalog_dac_Q, DAC_Q_pc] = PC_DAC(basebandDigital_Q,NbitsDAC(ii),Vref,dacType,basebandSamplingRate,continuousTimeSamplingRate);
    ...
    TXBB_Filt_pc = (10^(-10))*2*TXBB_Filt_Fcut*TXBB_Filt_Order; % Power consumption of the reconstruction filter
...
%TODO: consommation du PA
end

figure(1)
plot(NbitsDAC,ACPR_high)
xlabel('nombre de bits'); 
ylabel('ACPR');
title('ACPR en fonction de Nbits')
figure(2)
plot(NbitsDAC,Pow_useful_band)
xlabel('nombre de bits'); 
ylabel('Puissance utile');
title('Puissance utile en fonction de Nbits')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%                      Channel                        %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
basebandAnalog_amp   = rfLNA(rxSignal,LNA_Gain,LNA_NF,LNA_IIP3,Rin,continuousTimeSamplingRate/2); %Il sert à quoi?
