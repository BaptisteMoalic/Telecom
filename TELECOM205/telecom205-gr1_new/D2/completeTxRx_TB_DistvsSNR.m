...
%%%                      Channel                        %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
distance = [1.4 : 10 : 1410];
SNR_outputADC = zeros(1,length(distance));
...
for ii=1:length(distance)
    ...
    ChannelAttenuation = (c/carrierFreq./(4*pi*distance(ii)));
    ...
    bw_moitie = 10e6;
    SNR_outputADC(ii) = Calcul_SNR(basebandComplexSpectrum,adcSamplingRate,bw_moitie);
    ii
end

figure(1);
plot(distance, SNR_outputADC);
title('SNR en fonction de la distance');
xlabel('Distance (m)');
ylabel('SNR (dB)');
grid on;
