...
function retval = BER_multi_treshold(nb_err_treshold,BER_treshold,SNR_bit,N_bits,N,nb_iter_max,nb_err_min,const, M_const,H_toeplitz,No)
...
for ii = 1:length(SNR_bit);
  ...
  while ( ((nb_err_treshold(ii)< nb_err_min) && (jj < nb_iter_max )))   
    ...
    symboles_codes = (a_1.*symboles_codes_1).+(a_2.*symboles_codes_2).+(a_3.*symboles_codes_3);
