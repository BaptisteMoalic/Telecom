...
nb_err_treshold = zeros(1,length(L));
BER_treshold = zeros(1,length(L));
...
for ii = 1:length(L);
  ...
  while ( ((nb_err_treshold(ii)< nb_err_min) && (jj < nb_iter_max )))   
    ...
    fiber_output = fiber_model_new(output_TX, bitrate, L(ii));

    [output_RX, ~, ~, ~] = RX_photodetector(fiber_output, Ts_out/10, 0, photodetector_to_use_at_this_SNR);

    output_RX = abs(output_RX)/max(abs(output_RX)); % Normalizing the signal

    return_to_bits = zeros(1, N_bits);
    for kk = 0:N_bits-1
      if output_RX(10*kk+5)>(1/2) % Decision taken on the middle bit
...
xlabel('Length (km)')
ylabel('BER')
title('BER for an OOK non-coded @2.4Ghz')
