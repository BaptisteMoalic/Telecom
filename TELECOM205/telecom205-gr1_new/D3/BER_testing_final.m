...
for ii = 1:length(SNR_bit);
  ...
  while ( ((nb_err_treshold(ii)< nb_err_min) && (jj < nb_iter_max )))   
    ...
    [output_RX, ~, ~, ~] = RX_photodetector(fiber_output, Ts_out/9, 0, photodetector_to_use_at_this_SNR);

    output_RX = abs(output_RX)/max(abs(output_RX)); % Normalizing the signal

    return_to_bits = zeros(1, N_bits);
    for kk = 0:N_bits-1
      if output_RX(9*kk+5)>(1/2) % Decision taken on the middle bit
...
line([-21 -11], [10**(-3) 10**(-3)])
line([-21 -11], [10**(-6) 10**(-6)])
grid on
xlabel('P_opt (dBm)')
ylabel('BER')
title('BER for an OOK non-coded @10Ghz')
